...
from cvtda.topology import FiltrationsExtractor
...
def test_gray(num_objects):
    extractor = FiltrationsExtractor(settings=FiltrationsExtractor.PRESETS.quick, n_jobs=1)
    output = extractor.fit_transform(numpy.array([make_gray()] * num_objects))
    assert len(extractor.filtration_extractors_) == NUM_FILTRATIONS_QUICK
    assert output.shape == (num_objects, NUM_FILTRATIONS_QUICK * 32 * 2)
...
def test_rgb(num_objects):
    ...
    assert len(extractor.filtration_extractors_) == NUM_FILTRATIONS_QUICK
    assert output.shape == (num_objects, NUM_FILTRATIONS_QUICK * 32 * 2 * 4)
    assert numpy.isnan(output).sum() == 0
...
def test_transform_before_fit():
    ...
    with pytest.raises(AssertionError):
        FiltrationsExtractor(FiltrationsExtractor.PRESETS.quick, n_jobs=1).transform(input)


def test_dimensions_mismatch():
    input1 = numpy.array([make_gray()])
    extractor = FiltrationsExtractor(FiltrationsExtractor.PRESETS.quick, n_jobs=1).fit(input1)
