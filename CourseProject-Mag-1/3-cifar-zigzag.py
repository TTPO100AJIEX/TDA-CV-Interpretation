...
def run_model(
    ...
    *,
    ...
    train_ds = torchvision.datasets.CIFAR10('cifar-10', train = True, transform = transforms(), download = True)
    test_ds = torchvision.datasets.CIFAR10('cifar-10', train = False, transform = transforms(), download = False)

    train_y = torch.tensor([y for _, y in train_ds])
    test_y = torch.tensor([y for _, y in test_ds])
    ...
    dumper = zigzag.utils.UniversalDumper(f"zigzag_results/cifar-10/{model_name}")
